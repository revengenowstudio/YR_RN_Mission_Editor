...
BEGIN_MESSAGE_MAP(CAITriggerTypes, CDialog)
	...
	ON_CBN_KILLFOCUS(IDC_OWNER, OnEditchangeOwner)
	ON_CBN_KILLFOCUS(IDC_TEAMTYPE1, OnEditchangeTeamtype1)
	ON_CBN_KILLFOCUS(IDC_TEAMTYPE2, OnEditchangeTeamtype2)
	ON_CBN_KILLFOCUS(IDC_FLAG1, OnEditchangeFlag1)
	ON_CBN_KILLFOCUS(IDC_FLAG2, OnEditchangeFlag2)
	ON_CBN_KILLFOCUS(IDC_UNITTYPE, OnEditchangeUnittype)
	ON_CBN_KILLFOCUS(IDC_DATA, OnEditchangeData)
	ON_EN_KILLFOCUS(IDC_FLOAT1, OnChangeFloat1)
	ON_EN_KILLFOCUS(IDC_FLOAT2, OnChangeFloat2)
	ON_EN_KILLFOCUS(IDC_FLOAT3, OnChangeFloat3)
	ON_EN_KILLFOCUS(IDC_FLAG4, OnChangeFlag4)

	ON_BN_CLICKED(IDC_ENABLED, OnEnabled)
	ON_BN_CLICKED(IDC_ADD, OnAdd)
	ON_BN_CLICKED(IDC_DELETE, OnDelete)
	ON_CBN_KILLFOCUS(IDC_AITRIGGERTYPE, OnEditchangeAitriggertype)
	ON_CBN_SELCHANGE(IDC_CONDITION, OnSelchangeCondition)
	ON_EN_KILLFOCUS(IDC_NUMBER, OnChangeNumber)
